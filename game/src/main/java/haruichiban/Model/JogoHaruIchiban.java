...
public class JogoHaruIchiban {
    ...
    public boolean selecionarFlores(int indiceVermelho, int indiceAmarelo) {
        if (faseAtual != FaseJogo.SELECAO_FLOR) return false;
    ...
    public boolean executarFloracaoJunior() {
        if (faseAtual != FaseJogo.FLORACAO_JUNIOR) return false;
        
        Posicao nenufarEscuro = tabuleiro.encontrarNenufarEscuro();
        if (nenufarEscuro == null) return false;
    ...
    public boolean executarFloracaoSenior(int linha, int coluna) {
        if (faseAtual != FaseJogo.FLORACAO_SENIOR) return false;
    ...
    public boolean executarMovimentoNenufar(int linhaOrig, int colunaOrig,
                                           int linhaDest, int colunaDest) {
        if (faseAtual != FaseJogo.MOVIMENTO_NENUFARES) return false;
        
        if (!tabuleiro.moverNenufar(linhaOrig, colunaOrig, linhaDest, colunaDest)) {
